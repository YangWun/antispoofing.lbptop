...
def main():
  ...
  parser.add_argument('-n', '--normface-size', dest="normfacesize", default=[64,], type=int, help="this is the size of the normalized face box if face normalization is used. If more values are given, then the first one corresponds to height, and the second to the width (defaults to '%(default)s')", nargs='+')
